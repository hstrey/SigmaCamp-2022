...
# ╔═╡ 47c86ed4-1d0f-11ed-13cb-f7988c8c0bd9
begin
	using Plots
	using Random
	using Distributions
...
# ╔═╡ 18bffaad-6113-4272-a11a-5253d9bec27a
md"""
Before we start with automated differentiation, I would like to give some motivation why AD is important for data analysis applications and machine learning.  As an example, we consider some noisy data that we expect to lie on a straigt line.  The question is what is the optimal line through the data that takes into account all the information about the data.  Such an approach is often called maximum likelihood or for some special cases least-square fitting.

Let us create a line with known intercept and slope, add some noise and look at the data.
"""

# ╔═╡ ba878b43-a2b9-4238-9eb6-00da59646bab
datax = 0:7 # x

# ╔═╡ 5b6fa267-8c0f-4c3f-83df-5724ea81635a
d = Normal(0,3) # normal distribution with mean=0 and std=3

# ╔═╡ dbe76a00-ea4a-429d-9af6-332bf67d6670
md"""
Let me introduce the concept of least-square fitting of data.  The assumption that we are making here is that each data point that we are measuring is independent of each other and that each data point $y_{i}=f(x_{i},\theta)$ has an error that has been drawn from a normal distribution with zero mean and standard deviation $\sigma_{i}$. $\theta$ represents the parameters of the model $f(x,\theta)$ with which you want to fit the data.
...
# ╔═╡ 5d64de89-f519-4728-82b3-d2a833f4b0a1
md"""
64-bit floating point has about 16 digits of accuracy which can be expressed by the machine epsilon defined as $1 + \epsilon = 1$
"""

# ╔═╡ 9182a4c7-19aa-4906-b35e-3729087886d7
begin
	brange = -3:0.01:4
	mrange = 0:0.01:6
end

# ╔═╡ 71e57c94-428e-43ba-bd42-547fab88b6f0
md"""
The idea of least-square fit is to find the minimum of the Χ² function to determine the parameters of your model.  You can see above what this minimum is, but it is also clear that because of the errors of the data, these values don't necessarily agree with the true values.  For our example of a line fit, the minimum can be found by an analytic expression, but for more complicated models we need to find the minimum by numerical methods.

Let's think about some possible strategies:
"""

# ╔═╡ d4cd17c3-b61d-481f-8f30-ef8674efaf60
md"""
Above we show a parabula with the minimum at x=1.  Some strategies for finding this minimum are:
1) Pick N random points x, calculate f(x). The estimate for the minimum value is the smallest x value.  Not a very good strategy.  Converges poorly.
2) Pick an interval Δx and a starting point.  Evaluate f(x+Δx) and f(x-Δx) and then pick the point that is lower in value as the next x.  Converges slowly, but may get stuck at the bottom.  Also the closest that you can get to the minimum is Δx.
3) Take into account the derivative, or slope of f(x).  This is called the gradient decent method.  The step size is proportional to -f'(x).  Very good convergence, but can get stuck in local minima.
"""

# ╔═╡ 0e6a95b5-f106-4ec7-956e-08e2abf060d0
md"""
Now that we know that the gradient of a function is useful to find its minimum/maximum we have to discuss strategies to calculate the gradient.  One way is the finite difference method, but before we can discuss this method, we need to understand the accuracy of floating point numbers.
"""

# ╔═╡ 9a1ef7a5-7bc4-46f2-ba47-4dfaacab0a3b
md"""
...
# ╔═╡ 6576c01e-3a0e-4925-a10a-5b3311d116bd
eps(Float64)

# ╔═╡ f2cf6c75-9cae-4227-b0f6-6d64fd36a0ac
eps(0.1)

# ╔═╡ 288ffbd9-0c38-4509-a17f-4491f1059443
eps(0.01)

# ╔═╡ dd1b213b-2d79-4e03-9dd7-9ffe9727362a
md"""
...
# ╔═╡ b35ea9ec-77a4-4c94-b263-eea1ac0253da
md"""
...
# ╔═╡ 452bfda5-fc0d-4684-bf0f-5b9070410714
md"""
...
# ╔═╡ 157ee26b-03a4-437c-9df8-3df5b030b101
md"""
## Dual numbers
...
# ╔═╡ c625ee08-568c-4c80-ad37-e9fbec4e3f91
md"""
So now let's implement dual numbers in Julia
"""

# ╔═╡ 51e065d6-5e7e-48cd-a417-d3f4eefb1310
struct Dual
    val::Float64
    partial::Float64
end

# ╔═╡ 54dbfe9e-4a0b-47dd-9a0f-f57d9db7963e
val(x::Dual) = x.val

# ╔═╡ d80aecdf-9dbe-4326-a54d-de1f225b23a3
partial(x::Dual) = x.partial

# ╔═╡ 6cd6a882-da49-48a6-9cf6-2d41edb063c2
a = Dual(2,1)

# ╔═╡ 25f8cf9a-6cf6-44a7-b4b1-ac6cfd0980ca
val(a)

# ╔═╡ 7ce1350e-2543-447a-965e-7b21be40a52f
partial(a)

# ╔═╡ fdc8899e-d449-494a-a70d-455c4d3e4ef9
begin
	import Base: +, *
	+(f::Dual, g::Dual) = Dual(val(f) + val(g), partial(f) + partial(g))
	*(f::Dual, g::Dual) = Dual(val(f) * val(g), val(f)*partial(g) + val(g)*partial(f))
end

# ╔═╡ ce8086c1-c69c-49a3-84ee-483a36a7fb2a
begin
	Base.:+(f::Dual, α::Number) = Dual(f.val + α, f.partial)
	Base.:+(α::Number, f::Dual) = f + α
end

# ╔═╡ 2fa8236e-bd43-478a-8b75-2fba653c42e2
yline = 2.0 .+ 3 .* datax # true line

# ╔═╡ bfbab00b-517c-4495-b976-f8367dcef53f
ydata = yline .+ rand(d,length(datax)) # data is true line + errors

# ╔═╡ 4100b430-12f4-4027-b9c8-656724bceb31
begin
	scatter(datax,ydata, label="data y")
	plot!(datax, yline, label="true line",xlabel="x",ylabel="y")
end

# ╔═╡ ca37edc0-b35b-4f5e-9834-c14dccab2113
Base.:^(f::Dual, n::Integer) = Base.power_by_squaring(f,n)

# ╔═╡ 864ac15a-b5f5-4d94-bc3c-cec617df5cc5
function chisquare(b,m,datax,datay)
	liney = b .+ m .* datax
	error = datay .- liney
	return sum(error .^ 2 ./ 9)
end

# ╔═╡ 8627df8d-2e08-45a3-98bf-7b363d2f672d
chisquare(2,3,datax,ydata)

# ╔═╡ 05d05d14-0d03-46f8-941a-0c0492cac572
begin
	parax = 0:0.01:2
	paray = 2.0 .+ (parax .- 1.0) .^ 2
end

# ╔═╡ b5db6fcc-3480-46ae-bd05-ec8b7b424606
plot(parax,paray,label=nothing)

# ╔═╡ 50fe12ed-e759-4e87-9000-c7ccea68a523
dx = 10 .^ -(4:0.1:12)

# ╔═╡ 0e47593a-73b1-4d16-bbc3-92b6c2558f25
logdx = log10.(dx)

# ╔═╡ 870dd86b-6355-4a70-ab6e-47e42f5c3bc0
begin
	import Base.exp
	error = exp(1.0) .- (exp.(1.0 .+ dx) .- exp(1.0)) ./ dx
end

# ╔═╡ 0e775a90-2465-48b6-b2c0-6d154fb61140
chilh(b,m) = exp.(.- chisquare(b,m,datax,ydata))

# ╔═╡ 03f198f4-d7ae-4d2d-9c67-945ce494fc8b
contourf(brange,mrange,chilh,xlabel="b",ylabel="m")

# ╔═╡ 4c071bbd-8523-458e-8c79-1794f5814632
plot(logdx, log10.(abs.(error)),
	xlabel="log Δx",
	ylabel="log error",
	label = nothing,
	linewidth = 2)

# ╔═╡ 1972c9e3-ed50-416c-9507-ada2ca75c90e
md"""
...
# ╔═╡ 0dad0b59-04ee-483b-a680-93bee5794b29
begin
	Base.sin(f::Dual) = Dual(sin(val(f)), cos(val(f)) * partial(f))
end

# ╔═╡ 44767f71-3de0-4483-93a5-1648a438bf8d
sin(Dual(π,1))

# ╔═╡ 8a15fb76-f65b-4f9b-8a79-a55892beea43
(Dual(2.0,1.0))^3

# ╔═╡ b50f1513-2bbf-4dbb-9cf5-57d3e8311256
md"""
## The newest trend in automated differentiation: Enzyme

for this we need to understand how Julia is compiled.  Julia is using the llvm toolchain, which first generates pseudo-assembler, then optimizes before creating machine code.
"""

# ╔═╡ 51f06806-a238-4946-b0a3-1fd7e7ea9cff
f(x) = x^2

# ╔═╡ 48e9ac73-5b62-4275-8790-8540ba8ad94f
@code_llvm f(2)

# ╔═╡ 3ca2bbab-8883-44c2-80b6-94010395c4fc
@code_llvm f(2.0)

# ╔═╡ 9e7ee4dc-afde-4eba-9477-2497554b72c7
md"""
Enzyme implements automated differentiation on the level of pseudo-assembler, which allows a much more thorough opimization of code and often leads to speed increased from 5-20 fold.  It is implemented for Julia in Enzyme.jl
"""
