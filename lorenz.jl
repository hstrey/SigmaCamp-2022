...
# ╔═╡ d52b0d7e-1d8f-11ed-2b8d-bb7185bcf516
begin
...
# ╔═╡ 267ece83-95ae-428a-ad11-9a0e75e8859a
@variables t x(t) y(t) z(t)

# ╔═╡ f3398958-c8ff-45fc-9bb0-fd5d648379a1
@parameters β σ ρ

# ╔═╡ 7171828d-71b8-4365-928d-88ce42a33fc3
D = Differential(t)

# ╔═╡ f424e598-1837-49ee-995e-cfdb97b952db
eqs = [D(x) ~ σ * (y - x), D(y) ~ x * (ρ - z) - y, D(z) ~ x * y - β * z]

# ╔═╡ 32d97082-a469-4bb8-a906-8a8affdbc78a
@named lorenz = ODESystem(eqs, t)

# ╔═╡ f65e582a-1ba8-490b-b657-b11c8cb1d0c2
u₀ = [x => 1.0, y => 0.0, z => 0.0]

# ╔═╡ 737268b8-8158-40c0-a04f-8eb8d29ef3bd
p₀ = [σ => 10, β => 8/3, ρ => 28]

# ╔═╡ 224872d3-1dd7-4f60-aa49-ede626d78f78
tspan = (0.0, 100.0)

# ╔═╡ a1a4ee6f-280b-4be2-aea7-20dacbe4112f
prob = ODEProblem(lorenz, u₀, tspan, p₀)

# ╔═╡ d52fa52d-8102-410c-9ce1-177727793a17
sol = solve(prob, Tsit5())

# ╔═╡ 4defbd6b-b063-4863-b9b9-8fe70e0d0704
plot(sol, vars = (x,y,z))

# ╔═╡ 710712d0-e4b3-4d98-a3fe-3645df8ceb92
md"""
$(@bind sigma html"<input type=range min=1.0 max=20.0 step=1.0>")
$(@bind beta html"<input type=range min=1.0 max=10.0 step=0.333333333>")
$(@bind rho html"<input type=range min=10.0 max=50.0 step=1.0>")
"""

# ╔═╡ 6bc9bb71-866c-4261-81ae-652ee0607d96
p_new = [σ => sigma, β => beta, ρ => rho]

# ╔═╡ 1ae65668-ae48-4b57-9909-b681fe3e5050
prob_new = ODEProblem(lorenz, u₀, tspan, p_new)

# ╔═╡ 09016874-c2f3-4884-92f8-7975995da322
sol_new = solve(prob_new, Tsit5())

# ╔═╡ 1797cd8a-432c-4b11-b447-31835de49109
plot(sol_new, vars = (x,y,z))
